...
def writeInfo(torrentlinks, time, user, reputation, magnet):
    dailylog = open(os.path.join(abspath, 'Log_PirateBay_{}.txt'.format(today)), 'a')
...
# time is in GMT)
def timeCheck():
    dailylog = open(os.path.join(abspath, 'Log_PirateBay_{}.txt'.format(today)), 'r')
...
def cleanUp():
    dailylog = open(os.path.join(abspath, 'Log_PirateBay_{}.txt'.format(today)), 'r')
    temp = open(os.path.join(abspath, 'temp.txt'), 'w')
    for lines in dailylog:
    ...
    dailylog = open(os.path.join(abspath, 'Log_PirateBay_{}.txt'.format(today)), 'w')
    temp = open(os.path.join(abspath, 'temp.txt'), 'r')
    for lines in temp:
        line = json.loads(lines)
    ...
    dailylog.close()
    temp.close()
    os.remove(os.path.join(abspath,'temp.txt'))
...
if __name__ == "__main__":
    ...
    abspath = os.path.abspath('Users/sammishra/Documents/BitTorrent/ScrapingScripts/PirateBayScraping_v2')
